...
function Results({ userName, answers, timeTaken, onRetake }) {
  ...
  const sections = useMemo(() => {
    ...
    const numSections = Math.ceil(totalQuestions / SECTION_SIZE);
    for (let i = 0; i < numSections; i++) {
      const batch = results.slice(i * SECTION_SIZE, (i + 1) * SECTION_SIZE);
      const earned = batch.reduce((sum, r) => sum + r.earned, 0);
      const max = batch.reduce((sum, r) => sum + r.maxPts, 0);
      const startQ = 101 + i * SECTION_SIZE;
      const endQ = Math.min(100 + (i + 1) * SECTION_SIZE, 229);
      s.push({ label: `Q${startQ}-${endQ}`, earned, max });
    }
    return s;
  }, [results]);
  ...
  const handleShare = () => {
    const text = `SC-900 Practice Exam Part 2 (Q101-229) Result\nName: ${userName}\nScore: ${totalEarned}/${totalMax} (${pct}%)\nResult: ${passed ? 'PASS' : 'FAIL'}\nTime: ${formatTimeTaken()}`;
    ...
    <div className="results-container">
      ...
      <div className="results-header">
        ...
        <p className="result-subtitle">
          {passed
            ? 'You passed the SC-900 Practice Exam Part 2 (129 questions)!'
            : 'You need 70% to pass. Review the 129 questions below and try again!'}
        </p>
      </div>
